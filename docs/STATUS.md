...
> Atualizado por: Claude Code (DEEP_SECURITY V5.0 closeout — 37 issues fechadas em sec-hardening; 55 abertas restantes)
...
| Deep Security | V5.0 | 2026-04-19 | **55 ativas** (0 CRIT, 18 ALTO, 25 MEDIO, 12 BAIXO) — 16 CRITICOs fixados em 2026-04-19; **37 fechadas em 2026-04-25** (16 ALTO + 12 MEDIO + 9 BAIXO via commits sec-hardening `cd5044c`/`7b6593a`/`c209b56`/`c309a83`/`a8c377c`); 5 CVEs ainda ativos (multer, spring-boot) | [DEEP_SECURITY](audits/current/DEEP_SECURITY.md) |
...
- **Deep Security:** [DEEP_SECURITY](audits/current/DEEP_SECURITY.md) — **V5.0, 55 ativas (0 CRIT, 18 ALTO, 25 MEDIO, 12 BAIXO); 5 CVEs (multer/spring/vite) ainda pendentes**
...
| 2026-04-25 | **DEEP_SECURITY V5.0 closeout — 37 issues fechadas em 5 commits sec-hardening**. **16 ALTOs**: #008 (sync RBAC + audit log), #011 (HSTS/CSP/Referrer/Permissions-Policy na API), #013 (JWT revalidado em SUBSCRIBE/SEND no WS), #207 (Helmet + CSP no BFF), #208 (jwt.verify algorithms=HS256), #209 (sessionStorage + meta CSP no app), #210 (safeHttpUrl em PagamentoArtefato), #215 (Gemini prompt injection — sanitizeOcrPrompt + delimitador BEGIN_OCR/END_OCR), #216 (sslmode=disable nao mais forcado pelo Desktop), #411..#417 (nginx hardening — server_tokens, rate-limit login/api, timeouts, ciphers Mozilla intermediate, OCSP, Referrer/Permissions-Policy, CSP refinada). **12 MEDIOs**: #022 (jsonAuthEntryPoint), #023 (@JsonIgnore senhaHash), #027 (parseSegmento seguro), #220 (slug bound + LRU cap), #222 (randomUUID em multer), #225 (401-only logout), #226 (safeNotificationUrl no SW), #228 (NaN guard mes/ano), #229 (err.code em vez de err.message), #231 (logger newline-safe), #431 (pin minor Docker), #441 (actuator restrito a loopback). **9 BAIXOs**: #040 (handleDataIntegrity sem stack), #042 (LIMIT 1000), #044 (TRACE/CONNECT denyAll), #045 (logging hibernate=WARN), #046 (server.error.include-message=never), #234 (msg generica rate-limit), #235 (/health sem timestamp), #236 (bucket totp:), #451 (proxy_*_timeout). Otimizacao bonus: SecretKey JWT cacheada (`@PostConstruct`) — era recriada a cada parse. Helper `boundedCache` extraido (BFF). Skipados explicitos: #032 (PK schema), #038 (rawResponse review), #454 (delete arquivo). Commits: `cd5044c` (api), `7b6593a` (web), `c209b56` (app), `c309a83` (desktop), `a8c377c` (chore nginx+docker). **DEEP_SECURITY: 55 ativas** (0 CRIT, 18 ALTO, 25 MEDIO, 12 BAIXO + 5 CVEs deps). |
