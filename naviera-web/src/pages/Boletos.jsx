...
// #DP078: parcelas em paralelo (eram POSTs sequenciais com latencia O(qp) * RTT).
async function gerar(){if(!desc.trim()){showToast('Informe descricao','error');return}if(!valor||parseFloat(valor)<=0){showToast('Informe valor','error');return}if(!viagemAtiva){showToast('Selecione viagem','error');return}setSalvando(true);try{let idCat=null;if(catSel){const c=categorias.find(x=>x.nome===catSel);idCat=c?c.id:null}const vt=parseFloat(valor),qp=parseInt(parcelas)||1,vp=Math.round((vt/qp)*100)/100;const payloads=[];for(let i=0;i<qp;i++){payloads.push({id_viagem:viagemAtiva.id_viagem,descricao:qp>1?`${desc.trim().toUpperCase()} (${i+1}/${qp})`:desc.trim().toUpperCase(),valor_total:vp,valor_pago:0,data_vencimento:addMonths(dataVenc,i),id_categoria:idCat,forma_pagamento:'BOLETO',status:'PENDENTE',numero_parcela:i+1,total_parcelas:qp,observacoes:obs.trim()||null})}await Promise.all(payloads.map(p=>api.post('/financeiro/saida',p)));showToast(`${qp} boleto(s) gerado(s)!`);setDesc('');setValor('');setParcelas(1);setObs('');carregar()}catch(e){showToast(e.message||'Erro','error')}finally{setSalvando(false)}}
